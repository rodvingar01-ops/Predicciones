...
# --- CARGA DE DATOS OPTIMIZADA (CORREGIDA PARA LEER SOLO EL CSV NUEVO) ---
@st.cache_data(ttl=3600)
def cargar_datos():
    try:
        return pd.read_csv('Matches.csv', low_memory=False)
    except FileNotFoundError:
        st.error("Archivo 'Matches.csv' no encontrado. Verifica tu repositorio de GitHub.")
        return pd.DataFrame()
...
if not df.empty:
    ...
    # --- PESTAÑAS ---
    tab_predictivo, tab_base_datos = st.tabs(["🔮 Motor Predictivo", "🗄️ Forma Reciente (Últimos Partidos)"])
